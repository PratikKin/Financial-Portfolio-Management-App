...
const sendVerificationEmail = ({_id, email},res)=>{
    ...
    const mailOptions ={
        ...
        This Link will <b>expires in 6 hours</b></p><p>Click <a href=${currentUrl + "user/verify/" + _id + "/" + uniqueString}>here</a> to proceed.</p>`,
    ...
    .then((result)=>{
        ...
        if(result.length>0){
            ...
            if(expiredAt<Date.now()){
                ...
                .catch((error)=>{
                    console.log(error);
                    let message = "An error occurred while deleting expired unique string"
                    res.redirect(`/user/verified/error=true & message = ${message}`);
                })
            }
            else{
                ...
                .then((result)=>{
                    if(result){
                        ...
                        .then(()=>{
                            ...
                            .catch((error)=>{
                                console.log(error);
                                let message = "An error occurred while updating user detail to show verified."
                                res.redirect(`/user/verified?error=true&message=${message}`);
                        })
                        })
                        .catch((error)=>{
                        ...
                        let message = "Invalid verification deatil passed. Please check mail."
                        res.redirect(`/user/verified?error=true&message=${message}`);
                    }
                })
                .catch((error)=>{
                    console.log(error);
                    let message = "An error is occurred while comparing unique string"
                    res.redirect(`/user/verified?error=true&message=${message}`);
                })
            }
        }
            ...
            console.log(error);
            let message = "Account doesn't exist or verified earlier. Please Login or signUp"
            res.redirect(`/user/verified?error=true&message=${message}`);
        }
    })
    .catch((error)=>{
                    ...
                    if(result){
                        ...
                        .then((hashedNewPassword)=>{
                            ...
                            .catch(
                                ...
                                res.json({
                                    status:"Failed",
                                    message:"An error occurred while hashing updating newPassword."
                                })
                            }
                            )
...
const sendResetEmail = ({_id, email}, redirectUrl, res)=>{
    ...
    .then((result) =>{
        ...
        const mailOptions ={
            ...
            html:`<p>We heared you have lost your password.</p><p>Don't worry, use the below link to reset it </p><p>This Link will <b>expires in 60 minutes</b></p><p>Click <a href=${redirectUrl + "/" + _id + "/" + resetString}>here</a> to proceed.</p>`,
        };
